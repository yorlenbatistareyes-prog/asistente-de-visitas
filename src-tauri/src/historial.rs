...
pub fn obtener_historial_rust(congregacion_id: i64) -> Result<Vec<VisitaHistorialRust>, String> {
    ...
    let historial_iter = stmt
        .query_map(rusqlite::params![congregacion_id], |row| {
...
pub fn eliminar_historial_rust(id: i64) -> Result<(), String> {
    let conn = establecer_conexion().map_err(|e| e.to_string())?;
    conn.execute(
        "DELETE FROM historial_visitas WHERE id = ?1",
        rusqlite::params![id],
    )
    .map_err(|e| e.to_string())?;
    Ok(())
...
#[tauri::command]
pub fn obtener_desglose_ultimas_visitas_rust(
    circuito: String,
) -> Result<Vec<DesgloseUltimaVisita>, String> {
    let conn = establecer_conexion().map_err(|e| e.to_string())?;
    ...
    let rows = stmt
        .query_map(rusqlite::params![circuito], |row| {
            ...
            let datos =
                serde_json::from_str::<ContenidoInforme>(&contenido_limpio).unwrap_or_else(|_| {
                    ContenidoInforme {
                        publicadores: 0,
                        precursores: 0,
                        auxiliares: 0,
